"""
loading_screen.py — Full Iron Man Style Cinematic JARVIS Boot HUD
"""
...
class LoadingScreen(QWidget):
    finished = Signal()   # ← Fixed: Proper import + Signal
    ...
    def animate(self):
        ...
        if self.progress > 97 and self.phase == len(self.phases) - 1:
            QTimer.singleShot(1400, lambda: self.finished.emit())
    ...
    def paintEvent(self, event):
        ...
        # JARVIS Text
        p.setFont(QFont("Consolas", 52, QFont.Bold))
    ...
    def _corners(self, p, w, h):
        ...
        s = 45
        p.drawLine(20, 20, 20 + s, 20)
        p.drawLine(20, 20, 20, 20 + s)
        p.drawLine(w - 20, 20, w - 20 - s, 20)
        p.drawLine(w - 20, 20, w - 20, 20 + s)
        p.drawLine(20, h - 20, 20 + s, h - 20)
        p.drawLine(20, h - 20, 20, h - 20 - s)
        p.drawLine(w - 20, h - 20, w - 20 - s, h - 20)
        p.drawLine(w - 20, h - 20, w - 20, h - 20 - s)